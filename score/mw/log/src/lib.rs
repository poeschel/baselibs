...
use crate::mw_log_ffi::{
    mw_log_debug, mw_log_error, mw_log_fatal, mw_log_info, mw_log_verbose, mw_log_warn,
    mw_log_debug_additional, mw_log_error_additional, mw_log_fatal_additional, mw_log_info_additional, mw_log_verbose_additional, mw_log_warn_additional,
};
...
impl BufWriter {
    ...
    fn as_str(&self) -> &str {
        // Safety: We only write valid UTF-8 with fmt::Write
           ...
           if self.enabled(record.metadata()) {
               ...
               if SHOW_FILE || SHOW_LINE || SHOW_MODULE {
                   ...
                   match record.level() {
                       ...
                       Level::Debug => unsafe { mw_log_debug_additional(info.buf.as_ptr() as *const _, message.as_ptr() as *const _) },
                       Level::Trace => unsafe { mw_log_verbose_additional(info.buf.as_ptr() as *const _, message.as_ptr() as *const _) },
                   }
               } else {
                   match record.level() {
